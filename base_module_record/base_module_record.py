...
# Please do not override yaml_tag here: modify it in server
...
#            name=data.get('name','') or ''.lower()
    ...
    def _create_record(
        ...
        for key, val in data.items():
            if key in model_pool._columns.keys() and isinstance(
                    model_pool._columns[key],
                    openerp.osv.fields.function) and not model_pool._columns[key].store:
                continue
            if not (val or (fields[key]['type'] == 'boolean')):
                continue
            if (fields[key]['type'] in ('integer', 'float') or fields[
                    key]['type'] == 'selection' and isinstance(val, int)):
                ...
                if type(val) in (type(''), type(u'')):
                    ...
                    id, update = self._get_id(
                        cr, uid, fields[key]['relation'], val)
                ...
                if not id:
                    ...
                    name = relation_pool.read(
                        cr,
                        uid,
                        val,
                        [fld_nm])[fld_nm] or False
                    field.setAttribute(
                        "search", str([(str(fld_nm), '=', name)]))
                else:
                    ...
                    if valitem[0] in (0, 1):
                        ...
                        if valitem[0] == 0:
                            ...
                            newid, update = self._get_id(
                                cr, uid, fields[key]['relation'], valitem[1])
                            if not newid:
                                newid = self._create_id(
                                    cr,
                                    uid,
                                ...
                                valitem[1] = newid
                        self.blank_dict[
                            (fields[key]['relation'], valitem[1])] = newid

                        childrecord, update = self._create_record(
                            cr, uid, doc, fields[key]['relation'], valitem[2], newid)
                        noupdate = noupdate or update
                ...
                for valitem in (val or []):
                    if valitem[0] == 6:
                        for id2 in valitem[2]:
                            id, update = self._get_id(
                                cr, uid, fields[key]['relation'], id2)
                            self.blank_dict[
                                (fields[key]['relation'], id2)] = id
                            noupdate = noupdate or update
                            res.append(id)
                        field = doc.createElement('field')
                        field.setAttribute("name", key)
                        field.setAttribute(
                            "eval", "[(6,0,[" + ','.join(map(lambda x: "ref('%s')" % (x,), res)) + '])]')
                        record.appendChild(field)
            else:
                ...
                for valitem in (val or []):
                    if valitem[0] in (0, 1):
                        if key in model_pool._columns:
                            ...
                            fname = model_pool._inherit_fields[
                                key][2]._fields_id
    ...
    def _generate_object_xml(self, cr, uid, rec, recv, doc, result=None):
        ...
        if rec[3] == 'write':
            for id in rec[4]:
                ...
                if not id:
                    continue
                record, update = self._create_record(
                    cr, uid, doc, rec[2], rec[5], id)
                noupdate = noupdate or update
                record_list += record
            ...
            for id in rec[5]:
                ...
                if not id:
                    continue
                record, update = self._create_function(
                    cr, uid, doc, rec[3], rec[4], id)
                noupdate = noupdate or update
                record_list += record

        elif rec[3] == 'create':
            id = self._create_id(cr, uid, rec[2], rec[4])
            record, noupdate = self._create_record(
                cr, uid, doc, rec[2], rec[4], id)
            ...
            rec = copy_rec
            rec_data = [
                (self.recording_data[0][0],
                 rec,
                 self.recording_data[0][2],
                 self.recording_data[0][3])]
    ...
    def generate_xml(self, cr, uid):
        ...
        if len(self.recording_data):
            ...
            for rec in self.recording_data:
                if rec[0] == 'workflow':
                    rec_id, noupdate = self._get_id(
                        cr, uid, rec[1][2], rec[1][4])
                    if not rec_id:
                        continue
    ...
    def generate_yaml(self, cr, uid):
        ...
        if len(self.recording_data):
            ...
            for rec in self.recording_data:
                if rec[1][3] == 'create':
                    ...
                    continue
                if self.mode == "workflow":
                    record = self._generate_object_yaml(
                        cr,
                        uid,
                        rec[1],
                        rec[0])
                    yaml_file += "!comment Performing a workflow action %s on module %s" % (
                    ...
                    yaml_file += "!comment Performing an osv_memory action %s on module %s" % (
                        osv_action['action'], osv_action['model']) + '''\n'''
        ...
        for line in yaml_file.split('\n'):
            ...
            if (line.find('!record') == 0) or (
                    line.find('!workflow') == 0) or (line.find('!python') == 0):
                line = "- \n" + "  " + line
